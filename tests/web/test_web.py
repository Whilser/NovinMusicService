    ...
    def test_spa_and_assets_are_public_without_capturing_api_routes(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with TestClient(application) as client:
                ...
                apple_styles = client.get("/assets/apple.css")
            ...
            self.assertEqual(apple_styles.status_code, 200)
            self.assertIn(".detail-hero", apple_styles.text)
            self.assertIn(".service-status--green", apple_styles.text)
            self.assertIn("ICON_PATHS", script.text)
            self.assertIn("serviceHeading", script.text)
